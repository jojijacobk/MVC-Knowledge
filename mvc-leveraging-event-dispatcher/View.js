export default class View {
  constructor(eventEmitter, model) {
    this.eventEmitter = eventEmitter;
    this.model = model;
    this.app = this.getElement('#root-mvc-leveraging-event-dispatcher');
  ...
  bindEventHandlers() {
    this.getElement('#btnAddToDo').addEventListener(
      'click',
      function() {
        const txtAddToDo = this.getElement('#txtAddToDo').value;
        this.getElement('#txtAddToDo').value = '';
        this.eventEmitter.dispatch('addToDo', txtAddToDo);
      }.bind(this)
    );
  }
  ...
  bindCustomEvents() {
    this.eventEmitter.addListener(
      'itemAdded',
      function() {
        const listItems = this.model.getToDoList;
        this.displayToDoList(listItems);
      }.bind(this)
    );
  }
}
